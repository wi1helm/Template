...
public abstract class TemplateInventory {
    ...
    public TemplateInventory(Component name) {
        this.title = name;
        initialize();
    }
    ...
    public void fillInventory(TemplateItem filler) {
        for (int i = 0; i < 54; i++) { // Default inventory size to 54
            items.putIfAbsent(i, filler);
        }
    }

    public Inventory constructInventory(Player player, InventoryType type) {
